...
const StaffForm = () => {
    ...
    const [name, setName] = useState({firstName: "", lastName: "", approved: false});
    ...
    return (
        <Formik
            ...
            onSubmit={(values) => {
                request("https://sf-final-project-be.herokuapp.com/api/officers", "POST", JSON.stringify({...values, ...name}))
                    .then(data => {
        ...
        >
            <Form className="staff-form">
                ...
                <MyCheckBox
                    value={name.approved}
                    onClick={(event) => setName({...name, approved: event.target.checked})}
                    placeholder={<div className="staff-form_checkbox">Одобренный сотрудник</div>}
                />
                {error ? <div className="staff-form_error">Ошибка добавления, проверьте правильность введённых данных</div> : null}
