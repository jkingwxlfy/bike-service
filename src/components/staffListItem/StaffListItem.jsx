...
const StaffListItem = ({firstName, lastName, status, email, id, onDelete}) => {
    ...
    return (
        <div className="staff-list_item">
            ...
            <div className="staff-list_buttons">
                <NavLink to={`/staff/${id}`}><MyButton>Подробнее</MyButton></NavLink>
                {userId === id ? null : <MyButton onClick={() => onDelete(id)}>Удалить</MyButton>}
            </div>
        </div>
    )
}
